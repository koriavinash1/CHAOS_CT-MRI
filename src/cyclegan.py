...
D_A = Discriminator()
D_B = Discriminator()
D_SEG = FCDenseNet57(opt.nclasses, opt.channels)
...
optimizer_D_SEG = torch.optim.Adam(D_SEG.parametrs(), lr=opt.lr, betas=(opt.b1, opt.b2))
...
lr_scheduler_D_SEG = torch.optim.lr_scheduler.LambdaLR(optimizer_D_SEG, lr_lambda=LambdaLR(opt.n_epochs, opt.epoch, opt.decay_epoch).step)
